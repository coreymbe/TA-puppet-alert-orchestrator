...
import modalert_puppet_run_plan_helper

class AlertActionWorkerpuppet_run_plan(ModularAlertBase):

    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkerpuppet_run_plan, self).__init__(ta_name, alert_name)
    ...
    def validate_params(self):
        ...
        if not self.get_param("plan_name"):
            self.log_error('plan_name is a mandatory parameter, but its value is None.')
            return False
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_puppet_run_plan_helper.process_event(self, *args, **kwargs)
...
if __name__ == "__main__":
    exitcode = AlertActionWorkerpuppet_run_plan("TA-puppet-alert-actions", "puppet_run_plan").run(sys.argv)
    sys.exit(exitcode)
